...
from vimarc.dataset_readers.utils import char_span_to_token_span, split_tokens_by_hyphen
from vimarc.dataset_readers.vimarc import extract_answer_info_from_annotation, find_valid_spans

logger = logging.getLogger(__name__)


@DatasetReader.register("transformer_vimarc")
class TransformerVimarcReader(DatasetReader):
    """
    ...
    def _read(self, file_path: str):
        ...
        for context_id, context_info in dataset.items():
            ...
            for question_answer in context_info["qas"]:
                ...
                _, answer_texts = extract_answer_info_from_annotation(question_answer["answer"])
    ...
    def make_instances(
        ...
        answer_tokens = self._tokenizer.tokenize(answers[0])
        answer_tokens = split_tokens_by_hyphen(answer_tokens)
        tokenized_answer_text = [" ".join(token.text for token in answer_tokens)]
        valid_context_spans = (
            find_valid_spans(tokenized_context, tokenized_answer_text)
            if tokenized_answer_text
            else []
        )
        if not valid_context_spans:
            (token_answer_span_start, token_answer_span_end) = (-1, -1)
        else:
            first_answer_offset = valid_context_spans[0][0]
