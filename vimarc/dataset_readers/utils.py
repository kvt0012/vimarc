...
def char_span_to_token_span(
    ...
    """
    Converts a character span from a context into the corresponding token span in the tokenized
    version of the context.  If you pass in a character span that does not correspond to complete
    ...
    input character span.  If the tokenizer tokenized the context correctly and has matching
    ...
    if end_index == start_index and token_offsets[end_index][1] > character_span[1]:
        ...
        # cut off for some reason, like "split with Luckett and Rober", when the original context
...
def find_valid_answer_spans(
    ...
    """
    Finds a list of token spans in ``context_tokens`` that match the given ``answer_texts``.  This
    ...
    # step once.  This gives us a map from tokens to the position in the context they appear.
    word_positions: Dict[str, List[int]] = defaultdict(list)
    for i, token in enumerate(normalized_tokens):
        word_positions[token].append(i)
    spans = []
    for answer_text in answer_texts:
        # For each answer, we'll first find all valid start positions in the context.  Then
...
def make_reading_comprehension_instance(
    ...
    """
    Converts a question, a context, and an optional answer (or answers) to an ``Instance`` for use
    in a reading comprehension model.

    Creates an ``Instance`` with at least these fields: ``question`` and ``context``, both
    ...
    context_tokens : `List[Token]`
        An already-tokenized context that contains the answer to the given question.
    token_indexers : `Dict[str, TokenIndexer]`
        Determines how the question and context `TextFields` will be converted into tensors that
        get input to a model.  See :class:`TokenIndexer`.
    context_text : `str`
        The original context text.  We need this so that we can recover the actual span from the
        original context that the model predicts as the answer to the question.  This is used in
        official evaluation scripts.
    token_spans : `List[Tuple[int, int]]`, optional
        Indices into ``context_tokens`` to use as the answer to the question for training.  This is
        a list because there might be several possible correct answer spans in the context.
    ...
    answer_texts : `List[str]`, optional
        ...
        exactly one answer per question, but the dev and tests sets have several.  TriviaQA has many
    ...
    additional_metadata : `Dict[str, Any]`, optional
        The constructed ``metadata`` field will by default contain ``original_context``,
        ``token_offsets``, ``question_tokens``, ``context_tokens``, and ``answer_texts`` keys.  If
...
def make_reading_comprehension_instance_quac(
    ...
    token_span_lists : `List[List[Tuple[int, int]]]`, optional
        ...
        because there might be several possible correct answer spans in the context.
    ...
    num_context_answers : `int`, optional
        How many answers to encode into the context.
    additional_metadata : `Dict[str, Any]`, optional
        The constructed `metadata` field will by default contain `original_context`,
        ``token_offsets``, ``question_tokens``, ``context_tokens``, and ``answer_texts`` keys.  If
        you want any other metadata to be associated with each instance, you can pass that in here.
    ...
    def get_tag(i, i_name):
        # Generate a tag to mark previous answer span in the context.
        return "<{0:d}_{1:s}>".format(i, i_name)

    def mark_tag(span_start, span_end, context_tags, prev_answer_distance):
        try:
            assert span_start >= 0
            assert span_end >= 0
        except:  # noqa
        ...
        if span_start == span_end:
            context_tags[prev_answer_distance][span_start] = get_tag(prev_answer_distance, "")
        else:
            context_tags[prev_answer_distance][span_start] = get_tag(prev_answer_distance, "start")
            context_tags[prev_answer_distance][span_end] = get_tag(prev_answer_distance, "end")
            for context_index in range(span_start + 1, span_end):
                context_tags[prev_answer_distance][context_index] = get_tag(
    ...
    if token_span_lists:
        ...
        for question_index, answer_span_lists in enumerate(token_span_lists):
            ...
            span_start_list.append(IndexField(span_start, context_field))
            span_end_list.append(IndexField(span_end, context_field))
            prev_answer_marker_lists = [
                ["O"] * len(context_tokens),
                ["O"] * len(context_tokens),
                ["O"] * len(context_tokens),
                ["O"] * len(context_tokens),
            ]
            ...
            if num_context_answers > 2:
                p3_answer_marker_list.append(
                    SequenceLabelField(
                        prev_answer_marker_lists[3], context_field, label_namespace="answer_tags"
                    )
                )
            if num_context_answers > 1:
                p2_answer_marker_list.append(
                    SequenceLabelField(
                        prev_answer_marker_lists[2], context_field, label_namespace="answer_tags"
                    )
                )
            if num_context_answers > 0:
                p1_answer_marker_list.append(
                    SequenceLabelField(
                        prev_answer_marker_lists[1], context_field, label_namespace="answer_tags"
                    )
                )
