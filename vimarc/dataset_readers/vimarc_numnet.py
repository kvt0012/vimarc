...
from vimarc.dataset_readers.utils import (IGNORED_TOKENS,
                                          STRIPPED_CHARACTERS,
...
@DatasetReader.register("vimarc_reader_for_numnet")
class DropReader(DatasetReader):
    ...
    like.  The output formats all contain a question ``TextField``, a context ``TextField``, and
    ...
    give context spans as answers).  We typically return all possible ways of arriving at a given
    answer string, and expect models to marginalize over these possibilities.

    Parameters
    ----------
    tokenizer : ``Tokenizer``, optional (default=``WordTokenizer()``)
        We use this ``Tokenizer`` for both the question and the context.  See :class:`Tokenizer`.
    ...
    context_length_limit : ``int``, optional (default=None)
        If specified, we will cut the context if the length of context exceeds this limit.
    question_length_limit : ``int``, optional (default=None)
        If specified, we will cut the question if the length of context exceeds this limit.
    skip_when_all_empty: ``List[str]``, optional (default=None)
        ...
        skipped. Currently, you can put "context_span", "question_span", "addition_subtraction",
    ...
    instance_format: ``str``, optional (default="drop")
        ...
        various ways those numbers can be arrived at from the context, and return ``Fields``
        related to that.  "bert" format only allows context spans as answers, and provides a
        "question_and_context" field with the two pieces of text joined as BERT expects.
    ...
    def __init__(self,
        ...
        for item in self.skip_when_all_empty:
            assert item in ["context_span", "question_span", "addition_subtraction", "counting"], \
    ...
    def text_to_instance(self,  # type: ignore
                         ...
                         context_tokens: List[Token] = None) -> Union[Instance, None]:
        ...
        if self.instance_format == "squad":
            valid_context_spans = \
                self.find_valid_spans(context_tokens, tokenized_answer_texts) if tokenized_answer_texts else []
            ...
            for span in self.find_valid_spans(context_tokens, tokenized_answer_texts):
                ...
                valid_context_spans.append((span[0] + len(question_tokens) + 1,
                                            span[1] + len(question_tokens) + 1))
            if not valid_context_spans:
                if "context_span" in self.skip_when_all_empty:
                    return None
                else:
                    valid_context_spans.append((len(question_concat_context_tokens) - 1,
            ...
            all_number = numbers_in_context + numbers_in_question
            all_number_rank = get_number_rank(all_number)

            if all_number_rank is None:
                context_number_rank = []
                question_number_rank = []
            else:
                context_number_rank = all_number_rank[:len(numbers_in_context)]
                question_number_rank = all_number_rank[len(numbers_in_context):]

            number_indices = [indice + 1 for indice in number_indices]
            numbers_in_context.append(100)
            number_indices.append(0)
            context_number_rank.append(-1)
            ...
            question_number_rank.append(-1)

            context_number_rank = np.array(context_number_rank)
            question_number_rank = np.array(question_number_rank)
            ...
            if len(valid_context_spans) > 0:
                valid_question_spans = []

            target_numbers = []
            # `answer_texts` is a list of valid answers.
            for answer_text in answer_texts:
                number = self.convert_word_to_number(answer_text, True)
                if number is not None:
            ...
            if answer_type in ["number", "date"]:
                ...
                valid_signs_for_add_sub_expressions = self.find_valid_add_sub_expressions(numbers_in_context,
                                                                                          target_number_strs)
            ...
            type_to_answer_map = {"context_span": valid_context_spans,
    ...
    def make_marginal_drop_instance(question_tokens: List[Token],
                                    ...
                                    context_number_rank: np.ndarray,
                                    question_number_rank: np.ndarray,
        ...
        number_index_fields: List[Field] = [IndexField(index, context_field) for index in number_indices]
        fields["number_indices"] = ListField(number_index_fields)

        question_number_index_fields: List[Field] = [IndexField(index, question_field) for index in question_number_indices]
        fields["question_number_indices"] = ListField(question_number_index_fields)

        fields["context_number_rank"] = ArrayField(context_number_rank, -1)
        fields["question_number_rank"] = ArrayField(question_number_rank, -1)
        ...
        if answer_info:
            ...
            context_span_fields: List[Field] = \
                [SpanField(span[0], span[1], context_field) for span in answer_info["answer_context_spans"]]
    ...
    def make_bert_drop_instance(question_tokens: List[Token],
        ...
        metadata = {'original_context': context_text, 'context_token_offsets': context_offsets,
                    'question_tokens': [token.text for token in question_tokens],
                    'context_tokens': [token.text for token in context_tokens], }
    ...
    @staticmethod
    def find_valid_spans(context_tokens: List[Token],
                         answer_texts: List[str]) -> List[Tuple[int, int]]:
        normalized_tokens = [token.text.lower().strip(STRIPPED_CHARACTERS) for token in context_tokens]
    ...
    @staticmethod
    def find_valid_add_sub_expressions(numbers: List,
                                       targets: List,
