...
class ReadingComprehensionPredictor(Predictor):
    ...
    other model that takes a question and context as input.
    """
    ...
    def _json_to_instance(self, json_dict: JsonDict) -> Instance:
        """
        Expects JSON that looks like ``{"question": "...", "context": "..."}``.
        """
        question_text = json_dict["question"]
        context_text = json_dict["context"]
        return self._dataset_reader.text_to_instance(question_text, context_text)
    ...
    def predictions_to_labeled_instances(
        ...
        if "best_span" in outputs:
            ...
            context_field: SequenceField = new_instance["context"]  # type: ignore
            new_instance.add_field(
                "span_start", IndexField(int(span_start_label), context_field), self._model.vocab
            )
            new_instance.add_field(
                "span_end", IndexField(int(span_end_label), context_field), self._model.vocab
            )

        # For NAQANet model. It has the fields: answer_as_context_spans, answer_as_question_spans,
        # answer_as_add_sub_expressions, answer_as_counts. We need labels for all.
            ...
            # When the answer is in the context
                ...
                word_span_end = None
                offsets = new_instance["metadata"].metadata["context_token_offsets"]  # type: ignore
                ...
                context_field: SequenceField = new_instance["context"]  # type: ignore
                field = ListField([SpanField(word_span_start, word_span_end, context_field)])
                new_instance.add_field("answer_as_context_spans", field, self._model.vocab)

            # When the answer is an arithmetic calculation
            elif answer_type == "arithmetic":
                # The different numbers in the context that the model encounters
                sequence_labels = outputs["answer"]["numbers"]

                numbers_field: ListField = instance["number_indices"]  # type: ignore

                # The numbers in the context are given signs, that's what we are labeling here.
                ...
                # There's a dummy number added in the dataset reader to handle contexts with no
